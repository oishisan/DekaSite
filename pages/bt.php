...
if(isset($_POST['build']))
{
	...
	msquery("IF NOT EXISTS (SELECT name FROM %s.dbo.sysobjects WHERE name = 'tickets' and xtype = 'U') CREATE TABLE %s.dbo.tickets (tid varchar (50) PRIMARY KEY, type varchar (50), owner varchar (50), title varchar (20), status int, topen datetime, lby varchar (50) null)", $ini['MSSQL']['extrasDB'], $ini['MSSQL']['extrasDB']);
	// create ticket_post table
	msquery("IF NOT EXISTS (SELECT name FROM %s.dbo.sysobjects WHERE name = 'ticket_post' and xtype = 'U') CREATE TABLE %s.dbo.ticket_post (tid varchar (50), rid int, poster varchar (50), post text null, rdate datetime)", $ini['MSSQL']['extrasDB'], $ini['MSSQL']['extrasDB']);
