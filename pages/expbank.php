<?php
requireExtras();
echo '<center><table><tr><td class=header>Experience Bank</td></tr><tr><td class=submenu><a href=?do=',entScape($_GET['do']),'&type=bank target=_self>Visit Bank</a> | <a href=?do=',entScape($_GET['do']),'&type=gift target=_self>Gift Experience</a> | <a href=?do=',entScape($_GET['do']),'&type=list target=_self>List Experience</a> | <a href=?do=',entScape($_GET['do']),'&type=listing target=_self>Check Listings</a></td></tr></table>';
...
if( $_POST['type'] == 'Gift' && isset($_SESSION['token']) && $_POST['token'] == $_SESSION['token'] && !empty($_POST['sendTo']) && !empty($_POST['sendExp']))
{
	...
	if(ctype_digit($_POST['sendExp']))
	{
		...
		if ($count == 1)
		{
			...
			{
				$query2 = msquery("SELECT character.dbo.user_character.user_no from character.dbo.user_character where character.dbo.user_character.character_name = '%s'", $_POST['sendTo']);
	...
	if ($isAU == '1')
	{
		...
		if ($auInfo['aid'] <> $_SESSION['user_no'])
		{
			...
			if ($totalCoins >= $auInfo['coins'])
			{
				...
				msquery("UPDATE cash.dbo.user_cash SET amount = '%s' where user_no = '%s'", $totalCoins, $_SESSION['user_no']);
				msquery("DELETE FROM %s.dbo.blist where auctionID = '%s'", $ini['MSSQL']['extrasDB'], $auInfo['auctionID']);
				$sellQuery = msquery("SELECT amount, free_amount from cash.dbo.user_cash where user_no = '%s'", $auInfo['aid']);
