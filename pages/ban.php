...
if($_GET['action'] == 'ban' && !empty($_POST['dataname'])) 
{
	...
	if($bFetch['num'] == 1 && $bFetch['login_tag'] == 'Y')
	{
		...
		msquery("INSERT INTO %s.dbo.banned (wDate, accountname, reason, wBy, type) VALUES ('%s', '%s', '%s', '%s', 'b')", $ini['MSSQL']['extrasDB'], date('n/j/Y g:i:s A'), $accountname, $_POST['reason'], $_SESSION['webName']);
...
if($_GET['action'] == 'unban' && !empty($_GET['aid'])) 
{
	...
	if($uFetch['num'] == 1 && $uFetch['login_tag'] == 'N')
	{
		...
		msquery("INSERT INTO %s.dbo.banned (wDate, accountname, wBy, type) VALUES ('%s', '%s','%s', 'u')", $ini['MSSQL']['extrasDB'], date('n/j/Y g:i:s A'), $_GET['aid'] , $_SESSION['webName']);
